...
## 目录可见性（workspace catalog）

`tuner-workspace catalog` 用静态扫描（`method = ...` / `path = ...`）列出全部冻结资产，不需要任何装饰器；
`auth.py` / `registry.py` / `recording/` 等辅助模块会被跳过。API Object 在工作台只做只读展示，不从工作台发请求。
