...
- **目标**：本模板仓是"平台"，其公共资产（`.cursor/**`、`docs/spec/**`、`packages/**`、
  公共 `apps/**`、`tools/git-hooks/**`）更新时，作为 release 制品的 `init_repo` 要能
  同步版本。本 skill 把"漂移信号 → 语义化版本决策 → 刷新 manifest → release notes"落地。
- **写范围**：`pyproject.toml`（`[project] version`）、
  `apps/init_repo/release_manifest.json`、必要时相关组件 front-matter 的 `version`、
  以及 release notes（`docs/` 或 `CHANGELOG`）。

## 触发（何时用）

- `python -m apps.init_repo manifest --check` 报告 DRIFT（changed/added/removed）；
- 或 `pre-push` 钩子提示平台资产漂移；
- 或有意发布一个新版本模板给下游项目仓。

## 步骤

1. **看漂移**：`python -m apps.init_repo manifest --check`，列出变化的 tracked 文件。
...
3. **bump 版本**：更新 `pyproject.toml` 的 `[project] version`；对语义变化明显的
   `.cursor/**` 组件同步 bump 其 front-matter `version`（随后 `python -m apps.index_ai`
   刷新 `.cursor/REGISTRY.md`）。
4. **刷新 manifest**：`python -m apps.init_repo manifest --write`（会以新版本重算哈希）。
