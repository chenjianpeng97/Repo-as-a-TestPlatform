...
Layers mirror ``packages.api_test``:

- **definition** — :class:`~packages.api_mock.spec.RouteMock` /
  :class:`~packages.api_mock.spec.ResponseSpec`, the per-scenario response
  contract stored in ``data/mocks/**.json``.
- **state** — :class:`~packages.api_mock.store.MockStore`, disk baseline plus
  in-memory overrides written by the admin API.
- **serving** — :func:`~packages.api_mock.app.create_app`, a FastAPI app whose
...
def __getattr__(name: str) -> Any:
    """Defer the FastAPI import so ``packages.api_mock`` works without the extra."""
    if name == "create_app":
