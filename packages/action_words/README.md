...
uv run python -m packages.action_words run <word_id> --example
uv run python -m packages.action_words run <word_id> --params "{\"count\": 3}"
...
```bash
uv run python -m packages.action_words.db_seed.create_example
```
...
```python
from packages.action_words.db_seed.create_example import CreateExample
...
```python
from packages.action_words import ActionContext
from packages.action_words.db_seed.create_example import CreateExample
...
1. **先查重**：`uv run python -m packages.action_words list`；
