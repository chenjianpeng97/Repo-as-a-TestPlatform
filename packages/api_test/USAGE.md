...
export_v1 = APIModel(
    ...,
    method="POST",
    path="/argon/autho/resultDetail/export",
    body_schema={...},
    body_format="form",
    ...
)
...
- In steps, parse Excel with **`packages.excel`** (e.g. `ExcelWorkbook.from_bytes(resp.content)` or `first_sheet_rows(resp.content)`), not in `packages/api_objects/`.

```python
from packages.excel import ExcelWorkbook, first_sheet_rows
