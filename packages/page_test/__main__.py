...
用法::
    ...
    uv run python -m packages.page_test list
    uv run python -m packages.page_test describe plane.login@v1
    uv run python -m packages.page_test validate
    uv run python -m packages.page_test catalog --out report/page_objects_catalog.json
    uv run python -m packages.page_test run plane.login@v1 --flow login --example --headed
    uv run python -m packages.page_test doctor plane.login@v1
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    if result.fallbacks:
        print(
            ...
            f"`python -m packages.page_test doctor {model.id}` 查看体检报告",
            file=sys.stderr,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="python -m packages.page_test")
