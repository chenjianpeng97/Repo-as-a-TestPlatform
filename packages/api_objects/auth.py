...
Env vars:
  - ``TEST_BASE_URL``     — API host (see ``packages.config``)
...
from packages.api_test.model import APIModel, ApiResponse
...
Prefer maintaining credentials in ``packages.api_objects.auth`` (repo package).
This file is only created when recorder writes outside the default package tree.
"""
from packages.api_objects.auth import *  # noqa: F403
'''
