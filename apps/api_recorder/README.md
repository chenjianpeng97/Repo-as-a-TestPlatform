...
与 APIObject 请用 `python -m apps.recorder`。

冻结内核在 `packages.api_objects.recording`（离线可测，不含 mitmproxy / Playwright）。
...
python -m apps.api_recorder --outputs_dir D:\tmp\api_objects
python -m apps.api_recorder --include_host example.com --port 8888
...
合录入口同样支持 `--write-mocks`：`python -m apps.recorder --app <app> --write-mocks`。
...
```bash
uv run --extra test pytest packages/tests/test_api_objects_recording_*.py apps/api_recorder/tests -q
```
