"""CLI for the api_mock server.
    ...
    python -m apps.mock_server seed --dry-run
    python -m apps.mock_server routes

Import-safe on purpose: ``build_parser`` must be reachable from
``apps/mock_server/plane.py`` without importing fastapi/uvicorn, so the Plane
...
def _default_mocks_dir() -> str:
    return str(_repo_root() / "data" / "mocks")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m apps.mock_server",
        description=(
...
def _cmd_serve(args: argparse.Namespace) -> int:
    ...
    from packages.api_mock import create_app
...
def _cmd_seed(args: argparse.Namespace) -> int:
    from apps.mock_server.seed import seed_mocks
...
def _cmd_routes(args: argparse.Namespace) -> int:
    from packages.api_mock import MockStore
