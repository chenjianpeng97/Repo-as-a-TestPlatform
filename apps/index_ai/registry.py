...
REGISTRY_PATH = CURSOR_DIR / "REGISTRY.md"
...
def collect_rules() -> list[Component]:
    out: list[Component] = []
    for path in sorted((CURSOR_DIR / "rules").glob("*.mdc")):
...
def _collect_named(subdir: str, pattern: str, kind: str, trigger: str) -> list[Component]:
    out: list[Component] = []
    for path in sorted((CURSOR_DIR / subdir).glob(pattern)):
        fm = parse_front_matter(path.read_text(encoding="utf-8"))
        out.append(Component(
            kind=kind,
...
def write() -> pathlib.Path:
    REGISTRY_PATH.write_text(render(), encoding="utf-8", newline="\n")
...
def is_current() -> bool:
    if not REGISTRY_PATH.exists():
        return False
    return REGISTRY_PATH.read_text(encoding="utf-8") == render()
