"""PageModel 资产与不可变调用链 —— 对齐 ``tuner_testkit.api_test.model``。
...
def normalize_url_path(path: str) -> str:
    ...
    与 ``tuner_testkit.api_objects.recording.normalize_path`` 对 API path 的归一同构，供 fingerprint 去重使用。
    """
    segments = []
...
class PageModel:
    ...
    name: str
    description: str = ""
    #: 无 host 的路径；host 由 ``tuner_testkit.config.get_ui_base_url()`` 运行时注入
    url_path: str = "/"
