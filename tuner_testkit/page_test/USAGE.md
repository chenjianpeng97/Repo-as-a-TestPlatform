...
与 `tuner_testkit.api_test` 同构的三层：**冻结的声明式资产** → **不可变链式调用** →
**执行引擎独占 IO/日志**。对位关系：
...
```python
from tuner_testkit.page_test import ElementSpec, LocatorSpec
...
```python
from tuner_testkit.page_test import PageDriver
...
```python
from tuner_testkit.page_test import BasePage
...
`tuner_testkit.page_test.session` 集中管理凭据（对标 `api_objects.auth`），让单个资产文件
可以直接跑。在资产文件末尾加：
...
if __name__ == "__main__":
    ...
    from tuner_testkit.page_test.session import replay_flow, report
...
`driver` 惰性 import playwright，`tuner_testkit.page_test.testing` 提供 FakePage 替身，
...
交接见 `tuner_testkit/apps/page_recorder/README.md`。这不是 MCP capture 的替代品。
