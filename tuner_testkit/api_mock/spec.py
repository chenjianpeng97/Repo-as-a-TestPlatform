...
carries top-level key names (see ``tuner_testkit.api_objects.recording.codegen.build_response_hints``)
and its asserts are assertions, not a response definition. Keeping mock bodies
in ``data/mocks/`` therefore leaves ``tuner_testkit.api_test`` untouched and keeps
