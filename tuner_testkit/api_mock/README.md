...
mock HTTP 服务。工具入口与完整使用说明见 [`tuner_testkit/apps/mock_server/README.md`](../../tuner_testkit/apps/mock_server/README.md)；
本文件只讲**这个组件包的分层与 API**。
...
- `response_hints` 由 `tuner_testkit.api_objects.recording.codegen.build_response_hints` 生成，只有
...
**`tuner_testkit/api_test` 一行未改**，recorder 也不用改。
...
```python
from tuner_testkit.api_mock import MockStore, ResponseSpec, create_app

store = MockStore("data/mocks")
app = create_app(store=store)           # 需要 mock extra
```

不装 `mock` extra 时，`import tuner_testkit.api_mock` 依然可用（`create_app` 是
PEP 562 惰性导出），所以 `spec` / `store` / `router` 可以被普通单测直接引用。

纯内存构造，不碰磁盘：

```python
from tuner_testkit.api_mock import MockStore, ResponseSpec
...
- 不校验请求是否符合资产的 `body_schema` / `headers_policy`（那是 `tuner_testkit.api_test` 的事）。
- 不发真实请求、不连数据库；`create_app` 之外没有 IO。
- 日志走 `tuner_testkit.logging`，落盘前会对疑似凭证字段告警（见 `spec.scan_sensitive`）。
