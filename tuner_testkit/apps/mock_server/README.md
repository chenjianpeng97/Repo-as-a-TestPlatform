...
它的 `response_hints` 由 `tuner_testkit.api_objects.recording` 生成，只有顶层键名，**没有响应体、状态码、响应头**，
...
`tuner_testkit/api_test` 一行未改。消费侧也零改动——资产的 `path` 不含 host，
...
#    或合录：python -m tuner_testkit.apps.recorder --app <app> --write-mocks
#    b. 从已冻结资产的【截断】样例补骨架
python -m tuner_testkit.apps.mock_server seed --dry-run     # 先看会写什么
...
```bash
python -m tuner_testkit.apps.mock_server routes            # 资产 + mock 定义对照表
python -m tuner_testkit.apps.mock_server routes --json
```
...
api_mock serving on http://127.0.0.1:8931
  ...
  definitions   : <project>/data/mocks
...
- `path` 支持 `{id}` / `{uuid}` 动态段，语义与 `tuner_testkit.api_objects.recording.normalize_path` 一致。
...
| 来源 | 命令 | 数据完整度 |
| --- | --- | --- |
| **recorder 直存**（推荐） | `python -m tuner_testkit.apps.api_recorder --write-mocks` 或合录 `--write-mocks` | **完整**：全部行、全部层级、长字符串不裁 |
| **从资产 seed**（兜底） | `python -m tuner_testkit.apps.mock_server seed` | 截断：5 行 / 6 层 / 240 字符 |
...
- `data/mocks/` 不在 `tuner_testkit/apps/init_repo` 的 `PLATFORM_PATHS` 里 —— mock 数据是项目资产，
  不随平台 DNA 分发；`tuner_testkit/api_mock` 与本工具会分发。
