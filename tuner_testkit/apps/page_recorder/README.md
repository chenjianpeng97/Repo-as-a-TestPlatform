...
拉起 headed Playwright 浏览器，人工点击 / 填写 / 浏览时**直播冻结**声明式 `PageModel` 到 `packages/page_objects/`（元素表多候选 + 当前操作流）。对标合录入口 `apps.recorder` 的「抓一条冻一条」（本工具只冻 UI）。不走 `playwright codegen`（后者吐裸 locator，没有 fallback、没有 `{{password}}`、也不过 `locator_policy`）。同一会话还要冻 API Object 请直接用 `python -m tuner_testkit.apps.recorder`。

## 需求背景

`tuner_testkit.page_test` 已经把页面资产做成可序列化数据（`LocatorSpec` / `PageFlow` / `Step.to_dict()`），缺的是「浏览器会话 → Capture → merge 源码」这一层。工程师对着真实页面点一遍，就能得到可 `python -m tuner_testkit.page_test run` 的资产，而不是事后手写定位器。
...
uv run python -m tuner_testkit.apps.page_recorder --app plane --flow login
# 指定入口，并在每个新 URL 扫描可见控件
uv run python -m tuner_testkit.apps.page_recorder --app plane --flow login --url /sign-in --scan
# 只冻某个 host
uv run python -m tuner_testkit.apps.page_recorder --app plane --include_host example.com --url https://app.example/sign-in
```
...
回放（凭据走环境变量，见 `tuner_testkit.page_test.session`）：
...
# 或
uv run python -m tuner_testkit.page_test run plane.sign_in@v1 --flow login --example
```
