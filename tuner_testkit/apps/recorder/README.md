...
uv run python -m tuner_testkit.apps.recorder --app plane --flow login --url /sign-in --scan
# 只要 UI / 只要 API
uv run python -m tuner_testkit.apps.recorder --app plane --url /sign-in --page-only
uv run python -m tuner_testkit.apps.recorder --app plane --url /sign-in --api-only
# API 侧同时写完整 mock
uv run python -m tuner_testkit.apps.recorder --app plane --url /sign-in --write-mocks
```

旧命令 `python -m tuner_testkit.apps.recorder --port 8080` **不会**静默转去代理，会报错并提示改用
`python -m tuner_testkit.apps.api_recorder`。
...
- API：`apps.api_recorder.PlaywrightApiTap` → `tuner_testkit.api_objects.recording`
- tap 回调内禁止 `page.evaluate`（避免与 `expose_binding` 死锁）；可读 `response.body()`

```bash
uv run --extra test pytest tuner_testkit/apps/recorder/tests -q
uv run python -m tuner_testkit.apps.recorder --help
```
